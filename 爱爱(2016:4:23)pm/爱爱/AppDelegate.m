...
#define FirstEnterNum   @"click28"


#import "ZWIntroductionViewController.h"//引导界面库
@interface AppDelegate ()<RESideMenuDelegate,UITabBarControllerDelegate>
{
...
-(void)CreateSqlite
{
    ...
    if ([self.db open]) {
        ...
        NSString *creatBrowsHistoryTable =@"create table if not exists BrowsHistoryTable(newsId text,contentType integer,title text,titleImg blob,publishTime text,source text,browsTime text)";//浏览历史表

...
-(void)creatBaseControll        //加载控件
{
    ...
    getTestCode.titleLabel.font=[UIFont systemFontOfSize:14];//修改字体大小
...
//    bandPhone.layer.shadowColor=[UIColor blackColor].CGColor;
//    bandPhone.layer.shadowOffset=CGSizeMake(0, 1);
//    bandPhone.layer.shadowOpacity=0.5;
...
-(IBAction)touchToBandPhone:(UIButton *)sender  //绑定手机点击事件
{
    ...
    //获取首位字符
    NSString *firstCharacter = [phoneNum.text substringToIndex:1];
    
    if (phoneNum.text.length!=11||![firstCharacter isEqualToString:@"1"]) {

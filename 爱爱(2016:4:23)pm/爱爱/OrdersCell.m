...
-(void)setup    //加载基本界面
{
    ...
    _cancelOrder.titleLabel.font=[UIFont systemFontOfSize:15];
...
-(void)processTheRichtext:(UILabel *)label AndHead:(NSString *)head Andbody:(NSString *)days AndFoot:(NSString *)foot
{
    ...
    //添加属性
    [attributedString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:TitleTextColorNormal} range:NSMakeRange(0, [attributedString length])];
    
    //创建身体
    NSMutableAttributedString *bodyContent =[[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@" %@ ",days]];
    //添加属性
    [bodyContent addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15],NSForegroundColorAttributeName:MainThemeColor} range:NSMakeRange(0, [bodyContent length])];
    
    //创建尾部
    NSMutableAttributedString *footContent =[[NSMutableAttributedString alloc]initWithString:foot];
    //添加属性
    [footContent addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:TitleTextColorNormal} range:NSMakeRange(0, [footContent length])];
